...
const double BANDWIDTH = 20e6;  // 20 MHz for simplicity (Wi-Fi 6 supports 160 MHz, but we'll keep 20 MHz)
const double SUBCHANNEL_SIZE[] = {2e6, 4e6, 10e6};  // Subchannel sizes: 2 MHz, 4 MHz, 10 MHz
    ...
    double transmitPacket(FrequencyChannel& channel, int users, double subchannel_bandwidth) {
        ...
        while (true) {
            if (channel.isChannelFree(users)) {
                // User transmission time depends on the bandwidth of the assigned subchannel
                double user_data_rate = subchannel_bandwidth * MODULATION * CODING_RATE;
                double packet_time = PACKET_SIZE / user_data_rate; // transmission time for the packet on assigned subchannel
                latency += packet_time;
                return latency;
            } else {
...
public:
    ...
    void simulateCommunication(int users, int packets, double subchannel_bandwidth) {
        ...
        // Phase 3: Parallel Communication (Round-Robin Scheduling with OFDMA sub-channels)
...
int main() {
    ...
    for (int users : {1, 10, 100}) {
        ...
        // Simulate using 2 MHz, 4 MHz, and 10 MHz sub-channels
